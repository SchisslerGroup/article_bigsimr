...
```{r simBRCA, echo=TRUE, eval = FALSE, cache=TRUE}
## 3. Generate the simulated samples
## N <- 10
## system.time( simBRCA <- rvec(N, rho = rho[1:2, 1:2], params = brcaMargins[1:2], type = corType, adjustForDiscrete = FALSE) )
N <- nrow(brca)
system.time( simBRCA <- rvec(N, rho = rho, params = brcaMargins, cores = CORES, type = corType, adjustForDiscrete = FALSE) )
simBRCA[1:2, 1:2 ]
