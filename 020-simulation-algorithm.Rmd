...
1. Mapping step  
   i. Convert the target Spearman correlation matrix ${\bf R_{Spearman}}$ to the corresponding MVN Pearson correlation ${\bf R_X}$.   
   or  
i$^\prime$. Convert the target Kendall $\tau$ matrix ${\bf R_{Kendall}}$ to the corresponding MVN Pearson correlation ${\bf R_X}$.
2. Checking admissibility and nearest correlation computation
    (i) Check that ${\bf R_{X}}$ is a correlation matrix, a PSD matrix with 1's along the diagonal.   
	(ii) If ${\bf R_{X}}$ is a correlation matrix, the input matrix is *admissible* in this scheme.  
	Otherwise,  
(ii$^\prime$) Replace ${\bf R_{X}}$ with the nearest correlation matrix ${\bf \widetilde{R}_{X}}$, in the Frobenius norm.
3. NORTA step
	(i) Generate ${\bf X}=(X_1, \ldots, X_d) \sim N_d({\bf 0}, {\bf R_{X}})$.  
...
Our algorithm allows the generation high-dimensional data with arbitrary marginal distributions with a broad class of admissible Spearman correlation matrices and Kendall tau matrices. The admissible classes consist of the matrices that map to a Pearson correlation matrix. In particular, if we let X be MVN with N components and set

$S_P = \{ R_p : R_p is a correlation matrix for X \}$
$S_K = \{R_K : R_K is a Kendall tau matrix for a MVN \}$
$S_S = \{R_s : R_s is a Spearman correlation matrix for a MVN\}$ 

then there are 1-1 mappings between these sets. 

So if a user of your package says "I want a 100 dimensional RV with NB marginals where R_s is the target Spearman correlation matrix", you can easily check whether R_S is an element of the "admissible set" S_S. The same if they say they want the same marginals but a Kendall matrix R_K. As is, you check (at least based on paper) whether the Spearman satisfies 2-D bounds which is ok for 2-D problems. So you can provide higher dimensional information about whether something can be done using your algorithm. Furthermore, I would guess that these sets of admissible S_K and S_S are not that restrictive - In particular R_p approx R_s for a MVN so that would suggest the admissible set S_S is not very restrictive because R_p can be any psd matrix with 1's on diagonal.
