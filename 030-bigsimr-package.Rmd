...
This section describes a low-dimensional (2D) random vector simulation workflow via the `bigsimr` `R` package.
`bigsimr` provides an interface to the native code written in Julia, registered as the `Bigsimr` Julia package PROVIDE URL.
In addition to the native Julia `Bigsimr` package and `R` interface `bigsimr`, we also provide a python interface `bigsimr` (PROVIDE URL) that interfaces with the Julia `Bigsimr` package. 
