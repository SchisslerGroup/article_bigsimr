...
Though we don't know the true distribution of ozone levels or air temperatures, we'll assume that both are log-normally distributed since the values are positive and skewed.
We'll use the well-known maximum likelihood estimators for the lognormal parameters:

```{r ch030-aq-temp-pars, echo=FALSE}
df %>% 
  select(Temp) %>% 
...
margins <- alist(
  qlnorm(meanlog = 0.74, sdlog = 0.016),
...
margins <- mlist(
  qlnorm(meanlog = mle_mean(df$Temp), sdlog = mle_sd(df$Temp)),
...
Since our estimated Spearm correlation $\hat{ \rho_S }$ is within the theoretical bounds, the correlation is valid as input to `bigsimr:rvec`.
