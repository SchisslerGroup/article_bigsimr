...
\noindent Consider a bivariate example where we have a correlated $(Y_1, Y_2)^\top$ with $Y_1\sim N(\mu_1, \sigma_1^2)$ (normal) and $Y_2\sim LN(\mu_2, \sigma_2^2)$ (lognormal). 


\vspace{0.1in}


\noindent First, let us note that when we get such a vector from BigSimR package then in fact it can be represented as 
...
\item Return $(Y_1, Y_2)^\top$, where $Y_i=F_i^{-1}(U_i)$, $i=1,2$, where $F_i(\cdot)$ is the CDF of $Y_i$ and $F_i^{-1}(\cdot)$ is its inverse (the quantile function). 
...
where $c_2=\sigma_{Y_2}/\mu_{Y_2}$ is the {\it coefficient of variation} (CV) of the variable $Y_2$. Thus, the range of possible correlations in this model is not affected by the distribution of $Y_1$, and is determined by the CV of $Y_2$ as follows:
...
\end{equation}
 ...
 \noindent {\bf Further notes on the basic example in Section 4.1}. Based on the above analysis, one may propose a model of the form (\ref{kram1}) for these data, where $Y_1$ is the temperature (normal) and $Y_2$ is the Ozone variable (lognormal). Then, $X_1=Y_1$ and $X_2=\log Y_2$ will be bivariate normal, and the parameters can be estimated  based on the bivariate normal model. One can again use the marginal estimates as discussed in the paper, although one could actually use the exact same method for both variables (based on normality, after converting the Ozone variable to normal through the logarithmic transformation). According to the analysis presented in the paper, the values of 3.419 and 0.6967 are the estimates (the MLEs) of the mean and the variance of the underlying normal random variable $X_2$, that is $\hat{\mu}_2 = 0.3.419$ and $\hat{\sigma}_2=0.6967$. When this estimate of $\sigma_2$ is used in (\ref{kram8}), the factor on the far right becomes 
...
Thus, the possible range of correlation becomes $(-0.881, 0.881)$. This approximately agrees with the MC results in Section \@ref(package).
