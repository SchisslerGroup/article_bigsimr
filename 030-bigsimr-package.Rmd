...
The `bigsimr` package is a high-performance implementation of our proposed random vector generation algorithm and associated functions (see Section \@ref(algorithms)).
...
type <- 'spearman'
(R_S <- cor_fast(df, method = "spearman"))
```
...
cor_bounds(margins = margins, method = type)
```
