...
5. $\text{Return} \; Y \; \text{where} \; Y_i \gets F_{Y_i}^{-1}[\Phi(X_i)], \; i=1,...,d$.
...
2. Check admissibility and, if needed, compute the nearest correlation matrix.
  (i) Check that $R_X$ is a correlation matrix, a positive [semi]definite matrix with 1's along the diagonal.   
...
* Determine the input correlation, $\tilde{R}$, such that the resulting random samples using the NORTA algorithm have an estimated Pearson correlation that matches $R$
