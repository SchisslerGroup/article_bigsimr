...
```{r ch040-gpuVScpuFig, echo=FALSE, out.width='80%', fig.asp=1, fig.cap="Computation times as d increases."}
dat_long <- benchmark_dependences %>%
  ...
  mutate(dim_size = if_else(dim <= 1000, "Low Dimensional", "High Dimensional"),
         dim_size = factor(dim_size, levels = c("Low Dimensional", "High Dimensional")),
         dim = factor(dim),
         Step = factor(Step,
                       levels = c("corr_time",
                                  "adj_time",
...
ggplot(dat_long, aes(Correlation, Time, fill=Step)) +
  geom_bar(position = "stack", stat = "identity") +
  labs(y = "Time (Seconds)") +
  facet_grid(dim_size ~ Dimensions, scales = "free") +
