...
## save data as rds
saveRDS(brca, 'data/brca.rds')
```

```{r readBRCA, echo=TRUE, eval=TRUE}
## full workflow simulating RNA-seq data
brca <- readRDS(file = 'data/brca.rds') 
```
...
```{r runBRCA, results='none'}
n <- 20000
## d <- 100
## n <- 100
## d <- 5
nb_margins <- make_nbinom_alist(sizes, probs)
## compute theoretical bounds to make sure input is okay
## cor_bounds( nb_margins, type = type )
sim_nbinom <- rvec(n, nb_Rho, nb_margins, type = corType, ensure_PSD = TRUE, cores = CORES)
## sim_nbinom <- rvec(n, nb_Rho, nb_margins, type = "pearson", ensure_PSD = TRUE, cores = 4L)
## pairs(sim_nbinom)
nb_Rho_hat <- cor_fast(sim_nbinom, method = corType)
cat('The quadratic loss from the true correlation matrix is ') 
norm(nb_Rho - nb_Rho_hat, type = "2")
```
