...
As seen above $R_X$ is not strictly admissible in our scheme. This can be seen from the negative result from `bs$iscorrelation(R_X)`, which checks 1) positive-definiteness, 2) symmetry, and 3) 1s along the diagonal. However, the approximation is close with a maximum absolute error of `r max( abs(targets - approximates))` and average absolute error of `r mean(abs(targets - approximates))` across the `r choose(d,2)` correlations. With the inputs configured, `rvec` is executed to produce a synthetic RNA-seq data set:
...
```{r ch050-densityEvaluation, echo=T}
d <- ncol(sim_nbinom)
B <- nrow(sim_nbinom)
y0 <- 1
