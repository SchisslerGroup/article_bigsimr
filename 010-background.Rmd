...
This results in `r choose(d,2)` pairwise dependencies to characterize and simulate.
...
A central issue when describing multivariate distributinos is the choose of quantatitive description.
The most widely-known is the Pearson (product-moment) correlation coefficient that describes the linear association between two random variables $X$ and $Y$ and is given by
...
For non-normal marginals and monotone (possibly non-linear) correlation patterns, $\rho_P$ suffers some drawbacks and may mislead or fail to capture important relationships (@MK01).
Alternatively, in these settings, analysts often prefer rank-based correlatino measures to describe the degree of monotonic association.

Two nonparameteric, rank-based measures common in practice are Spearman's correlation (denoted $\rho_S$) and Kendall's $\tau$. 
...
Spearman's $\rho$ has an appealing correspondence as the Pearson's correlaiton coefficient on *ranks* of the values, thereby captures nonlinear yet monotone relationships.
...
By concordance we mean that orderings have the same direction if $X_i < X_j$, then $Y_i < Y_j$, and is determinded by the ranks of the values, not the values themselves (as in $\rho_P$).

Both $\tau$ and $\rho_S$ are **invariant to under monotone transformations** of the underlying random variates.
As we will describe more fully in the [Algorithms](#algorithms) section, this property is essential to our scheme to satistify properties SP1 and SP2, above.

This is no closed form, general correspondence among the rank-based measures and the Pearson correlation coefficent, as the marginal distributions $F_i$ are fundamental to their caclutain.
...
These facts are also critical in our simulation algorithm to broaden the dependency measures supported by `bigsimr`.

*Discrete marginal considerations*.
Spearman's correlation for discrete marginal suffers some issues due to the nonzero probability of ties (for example, the Spearman's correlation of a discrete-valued random variable $X$ could be less than 1; @MB13).
One remedy for this issue is to rescale Equation \@ref(eq:spearman). 
For two random variables $X,Y$ with pmfs (or pdfs) $p(x)$ and $q(y)$, respectively, define the rescaled Spearman's correlation as
...
Noting that with continuous marginals the rescaling returns $\rho_S$.
For discrete marginals with large or infinite support, computing adjustment factors $\sum_x p(x)^3$, $\sum_y p(y)^3$ over all large number of pairs becomes expensive (often violating desired property *SP1*).
And further the infinite sums must be approximately for count-valued data and potentially violating desired property *SP2*.

Also we'll note that for a discrete random variable $Y_i$, some care must be taken to define the quantile function $F_{i}^{-1}$.
Let 
...
Given two marginal distributions, the $\rho_P$ is not free to vary the entire range of possible correlations, $[-1,1]$.
These so-called *Frechet-Hoeffding bounds* are well-studied (for example, see @Nelsen2007 and @BF17).
This situatino gives strict restraints on the possible bounds and cannot be overcome through algorithm design.
...
There is a strong connection of our simulatino strategy to Gaussian *copulas* (for a technical introduction see @Nelsen2007).
A copula is a distribution function on $[0,1]^d$ describing a random vector with standard uniform marginals.
This definition provides a powerful, natural way characterize joint probability structure and the study of copulas is important in statistical theory and practice.

For  any random vector ${\bf X}=(X_1, \ldots, X_d)$ with cumulative distribution function (CDF) $F$ and marginal CDFs $F_i$ there is a copula function 
...
This representation corresponds to a multivariate normal distribution with standard normal marginal distributions and covariance matrix ${\bf R}$.
But since the marginals are standardized to have unit variance, this ${\bf R}$ is a Pearson correlation matrix. 
If $F_{{\bf R}}$ is the CDF of such multivariate normal distribution, then the corresponding Gaussian copula $C_{{\bf R}}$ is defined through
...
Sklar's Theorem [@Sklar1959; @Ubeda-Flores2017] guarantees that given inverse CDFs $F_i^{-1}$s and a valid correlation matrix (within the Frechet bounds) *any* random vector can be obtained via transformations involving copula functions. 
