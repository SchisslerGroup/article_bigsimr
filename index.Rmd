...
Further, efficient simulation empowers statistical computing strategies, such as the parametric bootstrap [@Rizzo2007] to simulate from a hypothesized null model, providing inference in analytically challenging settings.
...
For continuous non--normal multivariate data, the well-known NORmal To Anything (NORTA) algorithm [@Cario1997] and other copula [@Nelsen2007] approaches are well-studied with flexible, robust software available [@Yan2007; @Chen2001].
...
Further, we introduce the `bigsimr` R package that provides parallelized, high-performance software implemented our NORTA-inspired algorithm.
