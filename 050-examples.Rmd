...
In an illustration of our proposed methodology, we seek to simulate RNA-sequencing data by producing simulated random vectors with assumed marginal distributions and with estimated parameters that mimic the observed data and its generating process.
We seek realistic multivariate simulation to enable Monte Carlo (MC) methods for these data.
Modeling RNA-seq using multivariate probability distributions is natural as inter-gene correlation is an inherent part of biological processes [@Wang2009b].
...
See @Efron2012 for a detailed discussion with related methods and @Wu2012b, @Schissler2018, @Schissler2019 for applied examples.
The following subsections apply `bigsimr`'s methods to real RNA-seq data, including replicating an estimated parametric structure, MC probability estimation, and MC evaluation of correlation estimation efficiency.
...
Our initial goal is to replicate the structure of the TCGA BRCA RNA-seq data set (see [Background](background)).
Ultimately, we will simulate $B=10,000$ random vectors ${\bf Y}=(Y_1, \ldots, Y_d)^\top$ with $d=`r d`$.
Often researchers posit a negative binomial (NB) model as RNA-seq counts are often over-dispersed.
All $d$ selected genes exhibit over-dispersion (data not shown) and, so, we proceed to estimate the NB parameters $(r_i, p_i), i=1,\ldots,d$ to determine the target marginal PMFs $f_i$ (via the method of moments).
To complete specification of the simulation algorithm inputs, we estimate the Spearman correlation matrix ${ \bf R}_{Spearman}$ to characterize dependency.
...
We use the method of moments (MoM) to estimate the marginal parameters for the multivariate negative binomial model.
...
The functions below support this estimation for later use in `bigsimr::rvec`.
...
Notably, the marginal NB probabilities $\hat{p}_i's$ are small --- ranging in the interval $[`r min(probs)` , `r max(probs)`]$.
...
Now we check admissibility of specified correlation matrix.
We begin with the pairwise, 2D MC estimated bounds.
And then follow the `r d`D bounds.

```{r ch050-cor-check, echo=TRUE, eval=TRUE, cache = TRUE}
...
if ( ! is_valid_correlation( R_X ) ) {
    ## system.time( R_Xtilde <- cor_nearPSD( G = R_X )  )
    R_Xtilde <- cor_nearPSD( G = R_X )
    ## Quantify the error
...
## qplot( x = targets, y = approximates ) + geom_abline(slope=1,intercept = 0)
## qplot( x = targets - approximates ) + geom_abline(slope=1,intercept = 0)
summary(targets - approximates)
```

While the exact dxd Spearman correlation matrix is not strictly admissible in our scheme (seen by the non-PSD result above), the approximation is of sufficient quality for most applications with a maximum pairwise of `r max( targets - approximates)` and average error of `r mean( targets - approximates)`.
...
```{r ch050-makeSimDataFig, echo = FALSE, eval=T, cache=F, message=FALSE, warning=FALSE}
if( !file.exists("fig/ch050-simDataFig.png") ) {
    exampleGenes <- readRDS(file = './data/exampleGenes.rds')
    p <- GGally::ggpairs(
                     data = as.data.frame( sim_nbinom[ ,exampleGenes] ), 
                     upper = list(
                         continuous = GGally::wrap('cor', method = "spearman")
                     )) + theme_bw()
    ## ggsave("fig/ch050-simDataFig.png", plot = p, width = 5, height = 4, dpi = 300)
    ggsave("fig/ch050-simDataFig.png", plot = p, dpi = 300) 
}
```

```{r ch050-simDataFig, echo=F, fig.align='center', fig.cap='Simulated data for three selected high-expressing genes generally replicates the estimated data structure. The data do not exhibit outlying points, but do possess the desired Spearman correlations, central tendencies, and discrete values.'}
...
```{r ch050-makeFigBRCA, echo=F, eval=T, cache=F}
if( !file.exists("fig/ch050-figBRCA.png") ) {
    ## estimate rho from sims and plot
    R_S_hat <- cor_fast(sim_nbinom, method = type)
    ## triplot
...
The results show good simulation performance for our choice of parameters settings.
Our settings were motivated by modeling high-expressing genes from TCGA BRCA data set.
In general, the ability to match marginal and dependence parameters depends on the particular joint probability model.
We recommend to evaluate and tune your simulation until you can be assured of the accuracy.
...
```{r ch050-densityEvaluation, include=FALSE, eval=TRUE, cache=F}
d <- ncol(sim_nbinom)
B <- nrow(sim_nbinom)
threshold <- rep(1, d)
...
where ${\bf Y^{(b)} }$ is the $b^{th}$ simulated vector from a total of $B$ simulation replicates and $I ( )$ is the indicator function.
For example, we can estimate from our $B=10,000$ simulated vectors that the probability of all genes expressing (i.e., ${\bf y}_i \geq 1, \forall \; i$) is $`r pHat`$.

```{r ch050-densityEvaluationECHO, echo=TRUE, eval=TRUE, cache=F}
d <- ncol(sim_nbinom)
B <- nrow(sim_nbinom)
threshold <- rep(1, d)
...
To conclude the example applications, we demonstrate how `bigsimr` can be used to evaluate estimation efficiency.
In particular, we wish to assess the error in our correlation estimation above.
We used a conventional method, based on classical statistical theory, which unfortunately, was not designed for high-dimensional data.
Indeed, high-dimensional covariance estimation (and precision matrices) is an active area of statistical science (.e.g., @Won2013g and @VanWieringen2016).
...
simRho <- replicate(n = m, expr = {
  tmpSim <- rvec(n = N , R_X, nb_margins, cores = cores)
  bigsimr::cor_fast(x = tmpSim, method = type)
}, simplify = FALSE)
## find quadratic loss at each rep
quadLoss <- sapply(simRho, rags2ridges::loss, T = R_S, type = "quadratic")
```

```{r ch050-lossResults, echo=F, eval=T, cache=FALSE}
## quadLossRDS <- paste0( "results/brca", d, "_quadLoss.rds" )
...
if ( !file.exists( frobeniusRDS) )  {
    ...
    ## simRho[[1]][1:5,1:5]
    ## R_S[1:5,1:5]
    ## rags2ridges::loss(simRho[[1]], R_S, type = 'frobenius')
...
```{r ch050-loss-summary, echo=TRUE, eval=T, cache=FALSE}
summary(frobenius)
```
