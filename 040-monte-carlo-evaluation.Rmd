...
```{r biGamma, echo = FALSE, eval = TRUE}
...
for (i in 1:nrow(sim_pars)) {
    ...
    for (rho in cor_seq) {
        ...
        dir.create("./results/sims_gamma", showWarnings = FALSE)
...
```{r biGammaPlot, echo = FALSE, eval = TRUE, message=FALSE, warning=FALSE, fig.height = 5, fig.width= 8, fig.align='center', fig.cap = "`bigsimr` recovers the Pearson specified correlations for Bivariate Gamma."}
...
```{r biNegBin, echo = FALSE, eval = TRUE}
...
for (i in 1:nrow(sim_pars)) {
    ...
    for (rho in cor_seq) {
        ...
        dir.create("./results/nbinom_sims", showWarnings = FALSE)
...
```{r gpuVScpuFig, echo=F, out.width='80%', fig.align='center', fig.cap="Computation times as d increases. We filter to the top 1, 5, 10, 15, 20, 25\\% expressing genes (in terms of median expression.)"}
