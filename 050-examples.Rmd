...
While the exact $d \times d$ Spearman correlation matrix is not strictly admissible in our scheme (seen by the non-positive definite result above), the approximation is of sufficient quality for most applications with a maximum pairwise of `r max( abs(targets - approximates))` and average absolute error of `r mean(abs(targets - approximates))`.
...
if (!file.exists(frobenius_path)) {
    ...
    frobenius <- sapply(simRho, function(R) sum((R - R_S)^2))
    saveRDS(frobenius, frobenius_path)
}
