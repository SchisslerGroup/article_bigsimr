...
And, so, we'll focus the numerical experiments on assessing how well the procedure scales to high dimension with respect to reasonable computation times (property S1 above) and accurately matching marginal and dependency parameters. 
...
For all selected bivariate marginals, the Pearson correlation matching is approximate.
...
Yet generally the accuracy seems adequate for most applications, at least in these limited simulation settings.
...
The bivariate simulations provide a low-dimensional perspectitive.
But, of course, this is not `bigsimr`'s primary purpose.
We'd like the random vector methodolgy to scale to high-dimensional problems.

DESCRIBE WHAT WE DID. Do we need to redo this?
