...
At each iteration we estimate the quadratic loss from the specified $\bf{R^{Spearman}_Y}$ and average over the $m$ iteration.

```{r ch050-quadloss, echo=TRUE, eval=TRUE, cache=FALSE}
quadLossRDS <- paste0( "results/brca", round(myProb*100), "quadLoss.rds" )
if ( !file.exists( quadLossRDS) )  {
    n <- nrow(brca)
    simRho <- replicate(n = m, expr = { tmpSim <- rvec(n = nrow(brca) , nb_Rho, nb_margins,
                                                       type = corType, ensure_PSD = TRUE, cores = cores)
                                                       tmpRho <- bigsimr::cor_fast( x = tmpSim )} ,
                        simplify = FALSE)
...
```{r ch050-quadloss-summary, echo=TRUE, eval=TRUE, cache=FALSE}
summary(quadLoss)
```
