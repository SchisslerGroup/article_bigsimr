...
The `bigsimr` package is a high-performance implementation of the proposed random vector generation algorithm and associated functions (see Section \@ref(algorithms)).
When designing `bigsimr`, we aimed to conveniently provide parallelized computation, with multicore and GPU acceleration.

This section describes basic of `bigsimr`, by stepping through a low-dimensional (2D) simulation workflow for the often-used example data set `airquality`.
This workflow proceeds from data, to estimation, simulation configuration, and random vector generation.
For this low-dimensional setting, we compute using a single central processing unit (CPU) as the overhead in forking the tasks to multiple cores outways the computation gains. 
Then we transition to advanced use where we briefly describe some of the high-performance features and syntax.
The section concludes with a description of how to use `bigsimr` on a computer clusters through slurm scheduling via the `rslurm` package.
...
We're going to show the basic use and syntax of `bigsimr` by using the New York air quality data set (`airquality`) included in the R `datasets` package.
First we load the `bigsimr` library and a few other conveninent data science packages, including the syntaxically-pleasing `tidyverse` suite of R packages.
...
For simplicity and to provide a minimal worknig example, we'll consider bivariate simulation of temperature, in degrees Fahrenheit, and ozone level, in parts per billion.
...
Here the analyst is free to be creative and we will not take up goodness-of-fit considerations.
But it seems sensible without domain knowledge to estimate these quantative from the data and we have a suite of functions designed for this task.
...
Since our estimated correlation is within the theoretical bounds, the correlation is valid as input to `bigsimr:rvec`.
By comparison, the Pearson correlation is restricted for these margins.
