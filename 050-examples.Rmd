# RNA-seq data example applications  {#examples}
...
In an illustration of our proposed methodology applied to real data, we seek to simulate RNA-sequencing data by producing simulated random vectors with assumed marginal distributions with estimated parameters that mimick the observed data and its generating process.
We seek scaleable realistic multivariate simulation to enable Monte Carlo (MC) methods for thes data.
...
The following subsections apply `bigsimr`'s methods to real RNA-seq data, including replicated an estimated parametric structure, MC probability estimation, and MC evaluation of correlation estimation efficiency.
...
Our first goal is to replicate the structure of the TCGA BRCA RNA-seq data set (see [Background](background)).
...
The marginal distributions are from the same probability family (NB) yet are hetereogeneous in terms of the parameters probability and size $(p_i, n_i)$ for $i,\ldots,d$.
...
Simulated counts roughly mimick the observed data but with a smoothier appearance due to the assumed parameter form and with less extreme points then the observed data (c.f. Figure \@ref(fig:ch010-realDataFig).)

```{r ch050-simDataFig, echo = FALSE, eval=T, cache=F, message=FALSE, warning=FALSE, fig.width= 8, fig.align='center', fig.cap='Simulated data for 3 selected high-expressing genes, replicating the observed data structure.'}
