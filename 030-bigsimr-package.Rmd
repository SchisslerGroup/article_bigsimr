...
For this example, we do not take up goodness-of-fit considerations to determine the marginal distributions.
...
To avoid manual entry, `mlist` presents an alternative to `alist` with on-the-fly evaluation.
...
But we also support approximate Pearson correlation coefficient matching, while cautioning the user to check the performance for their parametric multivariate model (see [Monte Carlo evaluations](#simulations) below for evaluation strategies and guidance).
...
`bigsimr::cor_bounds` estimates the pairwise lower and upper correlation bounds, using the Generate, Sort, and Correlate algorithm of @DH2011.
