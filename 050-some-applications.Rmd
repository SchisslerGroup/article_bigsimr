...
```{r LoadLib050, echo = FALSE, eval = FALSE, message=FALSE, warning=FALSE, results = "hide"}
library(ggplot2)
library(tidyverse)
library(knitr)
library(dplyr)
...
```{r processBRCA, echo=FALSE, eval=FALSE, cache=TRUE}
...
```{r figNBratio, echo=FALSE, eval=FALSE, message=FALSE, warning=FALSE, out.width= '75%', fig.align='center', fig.cap='The genes display extreme overdispersion. Using group-specific sample means and variances for each of the d=`r d` genes, we display the distributions oflog of variance to mean ratio.'}
...
```{r estRhoBRCA, echo=FALSE, eval=FALSE}
## full workflow simulating RNA-seq data
...
```{r estMargins, echo=FALSE, eval=FALSE, cache=TRUE}
...
```{r simBRCA, echo=FALSE, eval=FALSE, cache=TRUE}
...
```{r evalBRCA, echo=FALSE, eval=FALSE, cache=TRUE}
...
```{r densityEvaluation, echo=FALSE, eval=FALSE}
## full workflow simulating RNA-seq data
