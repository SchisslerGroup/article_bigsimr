...
This subsections below describe the basic use of `bigsimr`, by stepping through a low-dimensional (2D) simulation workflow for the often-used example data set `airquality` that contains daily air quality measurements in New York, May to September 1973 [@Chambers1983].
...
It is a bit inconvenient to repeatedly fill in the parameter values manually, so we provide a convenience function `mlist` which behaves similarly to `alist`, except that it evaluates the right side of argument values within the list.
This is intended to help with scaling up your code to high dimensions when many marginals to specify.
...
These estimation methods are the standard approaches, not designed specifically designed for high-dimensional correlation estimation (see the [Conclusion and Discussion]({#discussion) sections for more on this).
...
To ensure that the simulation is not configured to impossible settings, the `bigsimr::cor_bounds` function provides MC estimated theoretical lower and upper bounds (using the Generate, Sort, and Correlate algorithm of @DH2011).
...
Though `bigsimr` runs quickly, for large $d$ users may wish to run jobs on a shared computing server.
