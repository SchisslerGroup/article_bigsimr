...
The following description is directed towards towards `R` users via the `bigsimr` `R` package. This package provides an interface to the native code written in Julia, registered as the `Bigsimr` Julia package PROVIDE URL In addition to the native Julia `Bigsimr` package and `R` interface `bigsimr`, we also provide a python interface `bigsimr` at PROVIDE URL that interfaces with the Julia `Bigsimr` package. The Julia implementation provides a high-performance implementation of our proposed random vector generation algorithm and associated functions (see Section \@ref(algorithms)). When designing `bigsimr`, we aimed to conveniently provide parallelized computation, through multi-core and GPU acceleration, while allowing advanced users to customize and automate workflows.


The subsections below describe the basic use of `bigsimr`, by stepping through a low-dimensional (2D) simulation workflow for the readily-available data set `airquality` that contains daily air quality measurements in New York, May to September 1973 [@Chambers1983]. This workflow proceeds from data, to estimation, simulation configuration, random vector generation, and result visualization. For this low-dimensional setting, we operate using a single central processing unit (CPU), as the overhead in forking the tasks to multiple cores outweighs the computational gains. Then we transition to advanced use where we briefly describe some of the high-performance features and syntax. The section concludes with a short description of how to use `bigsimr` on a computer clusters through slurm scheduling via the `rslurm` package [@R-rslurm].
...
As mentioned, the user must decide how to describe correlation, based on the particulars of the problem. For non-normal data and for improved simulation accuracy/scalability in our scheme, we advocate the use of Spearman's $\rho$ correlation matrix $R_S$ and Kendall's $\tau$ correlation matrix $R_K$. We also support approximate Pearson correlation coefficient matching, while cautioning the user to check the performance for their parametric multivariate model (see [Monte Carlo evaluations](#simulations) below for evaluation strategies and guidance). To aid in correlation specification, and estimation in general, we provide a high-performance function `cor_fast` which estimates Spearman, Kendall, or Pearson correlation of large data sets by computing pairwise correlations in parallel. (Anyone who has tried estimating Kendall's $\tau$ using `stats::cor` can attest that the routine does not scale to even moderate dimensions). These estimation methods are classical approaches, not designed for high-dimensional correlation estimation (see the [Conclusion and Discussion]({#discussion) sections for more on this).
...
Although `bigsimr` runs quickly, for large $d$ users may wish to run jobs on a shared computing server. The R package `rslurm` enables parallel `rvec` calls. This example assumes that `bigsimr` is installed on a system with a slurm scheduler. A single run of `rvec` using `rslurm` can be coded as:
...
<!-- 
Once your jobs complete, `rslurm` conveniently stores simulation results in a standardized format. Retriving the data is accomplished by
-->
...
The power of combining `bigsimr` and `rslurm` can be illustrated by simulating many correlation structures. The `rslurm::slumr_map` syntax mirrors the `base::lapply` and `purrr::map` functions. 
