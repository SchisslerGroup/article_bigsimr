...
authors:
  - name: A. Grant Schissler
    ...
    email: grant.schissler@gmail.com
  - name: Edward J. Bedrick
    affiliation: University of Arizona
  - name: Alexander D. Knudson
    affiliation: University of Nevada, Reno \AND
...
```{r ch030-aq-joint-dist, cache=FALSE, fig.width=6, fig.cap="Bivariate scatterplot of Ozone versus Temp with estimated marginal densities. The Ozone data are modeled marginally as log-normal and the Temperature data as normal.", eval=T, echo=F}
...
```{r ch030-plot-sim, fig.cap="Contour plot and marginal densities for the simulated bivariate distribution of Air Quality Temperatures and Ozone levels. The simulated points mimic the observed data with respect to both the marginal characteristics and bivariate association.", eval=T, echo=F, warning=F}
p1 <- df_sim %>%
...
```{r ch040-biNormPlot, eval=T, fig.height=5, fig.width=6, fig.cap="`bigsimr` recovers the Pearson specified correlations for MVN.", echo=F}
...
```{r ch040-biGammaPlot, eval=T, fig.height=5, fig.width=6, fig.cap="`bigsimr` recovers the Pearson specified correlations for Bivariate Gamma.", echo=F}
...
```{r ch040-biNegBinPlot, eval=T, fig.width=6, fig.height=5, fig.cap="`bigsimr` recovers the correlations for bivariate negative binomial only approximately for Pearson but (nearly) exactly for the rank-based correlations.", echo=F}
...
```{r ch040-largeDfig, fig.cap="Computation times for HD multivariate gamma simulation.", eval=T, echo=F}
# Set colors
numColors <- 5
numGroups <- 4
...
```{r ch050-simDataFig, cache=FALSE, fig.cap="Simulated data for three selected high-expressing genes generally replicates the estimated data structure. The data do not exhibit outlying points, but do possess the desired Spearman correlations, central tendencies, and discrete values.", eval=T, warning=F, echo=F}
set.seed(2020-02-25)
num_genes <- 3
gene_sample <- sample(example_genes[1:1000], num_genes)
